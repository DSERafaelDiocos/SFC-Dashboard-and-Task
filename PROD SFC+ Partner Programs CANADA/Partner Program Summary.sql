...
FROM PackageData
WHERE NAME = 'ABS-CBN Foundation International Canada'
GROUP BY Dates
...
SELECT
    ...
    TO_CHAR(COUNT(*), 'FM999,999,999') AS count
FROM PackageData
WHERE NAME = 'Gawad Kalinga Canada'
GROUP BY Dates
...
SELECT
    Dates,
    'SM Gift Pass Choice 2,000' AS Description,
    TO_CHAR(SUM(CAST(amount AS NUMERIC)), 'FM999,999,999.00') AS amount,
    TO_CHAR(COUNT(*), 'FM999,999,999') AS count
FROM PackageData
WHERE NAME = 'SM Gift Pass Choice Canada'
    AND item_name LIKE '%2,000.00%'
GROUP BY Dates
...
SELECT
    Dates,
    'SM Gift Pass Choice 5,000' AS Description,
    TO_CHAR(SUM(CAST(amount AS NUMERIC)), 'FM999,999,999.00') AS amount,
    TO_CHAR(COUNT(*), 'FM999,999,999') AS count
FROM PackageData
WHERE NAME = 'SM Gift Pass Choice Canada'
    AND item_name LIKE '%5,000.00%'
GROUP BY Dates
