WITH DUPE_HEADER AS (
    ...
    WHERE 
        (CUST_ID IS NULL OR CUST_ID LIKE '8366%')
    GROUP BY
        PROC_DATE, 
        DATETIME, EMP_NO, 
        TILL_NO, CUST_ID, 
...
SELECT 
    ...
    SUM(CASE WHEN CUST_ID LIKE '83%' AND CUST_ID NOT LIKE '%XXXXX%' THEN TOT_ITEMS ELSE 0 END) / COUNT(*) AS AVG_BASKET_SIZE_MEMBER,
    SUM(CASE WHEN CUST_ID IS NULL THEN TOT_ITEMS ELSE 0 END) / COUNT(*) AS AVG_BASKET_SIZE_NON_MEMBER,
    SUM(TOTAL) / COUNT(*) AS AVG_TOTAL_AMOUNT,
    ...
    -- SUM(CASE WHEN CUST_ID LIKE '83%' AND CUST_ID NOT LIKE '%XXXXX%' THEN TOTAL ELSE 0 END) / COUNT(*) AS AVG_BASKET_AMOUNT_MEMBER,
    -- SUM(CASE WHEN CUST_ID IS NULL THEN TOTAL ELSE 0 END) / COUNT(*) AS AVG_BASKET_AMOUNT_NON_MEMBER
FROM 
    DUPE_HEADER
WHERE
    (CUST_ID LIKE '83%' AND CUST_ID NOT LIKE '%XXXXX%') OR CUST_ID IS NULL
    -- CUST_ID = '8366880000001954' 
    -- AND DATETIME LIKE '1/27/2025 %'
    -- AND WEEK_OF = '11/04/2024'
GROUP BY
    WEEK_COUNT,
    WEEK_OF,
    STORE
