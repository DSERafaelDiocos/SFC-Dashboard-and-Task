...
SELECT 
    ...
    TO_CHAR(SUM(points), 'FM999,999,999,999') AS Points
FROM 
    "BPAY_SFCRWDS_transactions" t        
    INNER JOIN "BPAY_SFCRWDS_accounts" a 
...
WHERE        
    ...
    -- AND TO_CHAR(CAST(CONVERT_TIMEZONE('America/Los_Angeles', "timestamp") AS DATE), 'MM/DD/YYYY') <= CURRENT_DATE  -- Exclude today's date
GROUP BY 
